...
setMethod(f = "updateCovidData", 
          ...
          function(x){
            ...
            x@infectedPopulation <- x@infectedPopulation + x@susceptiblePopulationUnvaccinated*(x@infectedPopulation*x@rO*(1)/x@recoveryTime) + x@susceptiblePopulationVaccinated*(x@infectedPopulation*x@vaccineModifier*x@rO*(1)/x@recoveryTime) - x@infectedPopulation*1/x@recoveryTime
            x@changePerDay <- (x@infectedPopulation - infectedPopulationCurrent)*x@totalPopulation
...
covidTimeDataSetup <- function(covid, timeFrame){
  mat <- matrix(nrow = (timeFrame + covid@incubationPeriod), ncol = 5)
  ...
  mat[1:covid@incubationPeriod, 5] <- covid@changePerDay
  return(mat)
}
...
for(t in 1:timeFrame){
  ...
  #   next
  # }
  if((1 - covid@incubationPeriod) >= 1){#Noone is infected
    covidTimeData[t,1] <- t
    covidTimeData[t,2] <- 0
    covidTimeData[t,3] <- 1
    covidTimeData[t,4] <- 0
  ...
  covidTimeData[(t + covid@incubationPeriod),5] <- covid@changePerDay
}

#I think the time vs pop (infected) is plotted by distPlot
dat <- as.data.frame(covidTimeData)
colnames(dat) <- c("time", "infected", "susceptible", "pop", "changePerDay")
return(dat)
}
